...
def main():
    global game_speed, x_pos_bg, y_pos_bg, points, obstacles, death_count, state, font
    run = True
    ...
    def score():
        ...
        if int(points) % 5 == 0 and int(points) != 0 and int(points) <= 100:
            points += 0.01
            game_speed += 0.02
        text = font.render("Điểm số: " + str(int(points)), True, FONT_COLOR)
...
def game3():
    global points, death_count, state, font
    global FONT_COLOR
    run = True
    exit = 0
    while run:
        if state == 'play3':
            ...
            if death_count == 0:
                text = font.render("Nhấn một nút bất kỳ để bắt đầu", True, FONT_COLOR)
            elif death_count > 0:
                text = font.render("Nhấn một nút bất kỳ để chơi lại", True, FONT_COLOR)
                score = font.render("Điểm số của bạn: " + str(round(points)), True, FONT_COLOR)
